-- ==============================================================================
-- Supabase Database Schema: Book Explorer
-- ==============================================================================

-- 1. Ensure pgcrypto extension is available for UUID generation
...
CREATE TABLE IF NOT EXISTS public.books (
  ...
  reading_difficulty TEXT CHECK (reading_difficulty IN ('Beginner', 'Intermediate', 'Advanced')),
  estimated_reading_time TEXT DEFAULT '',
  cover_color TEXT DEFAULT 'from-zinc-800 to-zinc-950',
  ...
  share_assets JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 3. Indexes for fast search and autocomplete
...
-- Full-text search index across title, author, and genre
CREATE INDEX IF NOT EXISTS idx_books_fts ON public.books 
  USING gin (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(author, '') || ' ' || coalesce(genre, '')));
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'books' AND policyname = 'Allow public read access on books'
  ) THEN
...
-- 6. Trigger to automatically maintain updated_at timestamp
...
-- 7. Seed Data: Populate existing preset books (Non-destructive: ON CONFLICT DO NOTHING)
...
INSERT INTO public.books (
  ...
  'F. Scott Fitzgerald',
  '1925',
  'Classic Fiction, American Literature',
  4.4,
  180,
  'Intermediate',
  '3.5 hours',
  'from-emerald-800 to-zinc-905',
  'Set in the roaring twenties on Long Island, F. Scott Fitzgerald''s masterpiece is a devastating critique of the American Dream, disillusionment, and social class. Narrated by Nick Carraway, a midwestern transplant, the story follows the mysterious millionaire Jay Gatsby and his obsessive love for the beautiful, wealthy Daisy Buchanan. It is an exquisitely written exploration of romantic obsession, social decay, and the unyielding weight of the past.',
  TRUE,
  '[
